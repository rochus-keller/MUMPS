...
STORE   D ONEJOB V ST+278::0
        ...
        I D1 S D=$V(0,D1) F I=1:1 Q:'D  S ^("DDPOUT"_I)=$V(52,D)*65536+$V(50,D),^("DDPIN"_I)=$V(56,D)*65536+$V(54,D),^("DDPNAM"_I)=$V(10,D),D=$V(0,D)
        S ADD=80 F I=0:1:7 S ^RTH(SUB,"DISK",I,"READ")=$V(ADD+2,MM)*65536+$V(ADD,MM),^("WRITE")=$V(ADD+6,MM)*65536+$V(ADD+4,MM),ADD=ADD+8
...
NODE    S RTN="",U=$V(ADD,MM)#32,V=$V(ADD,MM)\32#8
        I '(U+V) S U=$V(ADD+14,MM),V=$V(ADD+16,MM),UCI=$C(U\2048#32+64,U\64#32+64,U\2#32+64)_","_$C(V\2048#32+64,V\64#32+64,V\2#32+64)
        E  S UCI=$ZU(U,V)
