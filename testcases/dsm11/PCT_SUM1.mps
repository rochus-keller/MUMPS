%SUM1   ;PARSE ROUTINE LINES : JEC ; 6-JUN-79  9:41 AM
        S A=0,Q=$C(34)
A1      S A=A+1,(U,M,P)=0 I $D(^UTILITY("SUM",%JB,%I,A)) S L=^(A),H=$F(L," ",1),W=0 D SB G A1
        K A,B,C,E,F,G,H,I,J,K,L,M,O,P,Q,R,S,T,U,V,W,X,Z
END     Q
...
SB1     S X=$E(L,H),R=$E(L,H,H+2),H=H+1 G SB1:X=" " Q:X=";"!(X="")!(X'?1A)  F H=H:1 Q:$E(L,H)'?1A
        ...
        G SB1:"Q"[X S B="WR"[X,G="BCFHIJKLNORSUVWX"[X,K="FKNORS"[X*2,M=X I X="V" S ^UTILITY("SUM1",%JB,%I,0,"VIEW")="",^UTILITY("SUM1",%JB,0,0,"VIEW")=""
...
S2      S O=B,F=G,S=K,(E,X)=0 I $E(L,H+1)'="@" D T2 G S3
        ...
        F O=1,4 I X=O!(X=5) S Z=M S:Z="Z" Z=$E(R,1,2) S IND=Z_" "_$E(L,E,H-1) S ^UTILITY("SUM1",%JB,%I,O,$E(IND,1,63))=IND
S3      G SB1:" ;"[C,S1
T1      G Q1:C=Q,W1:C?1A!(C="%"),N1:C?1N,F1:C="$",G1:C="^"
        I C="." F H=H+1:1 I $E(L,H)'?1A S C="?",O=0,H=H-1 Q
        G:'O V1:C="?" S:C="+"&'F F=1 S X=0 S:C=":" F=1
        I C="(" S:M="K"!(M="L")!(M="N") (K,S)=0 G T2:M="K"!(M="L")!("N") D T2 S X=1 S:"S"[M X=0
        G T2:C'="@"!E F J=H:-1:1 Q:$E(L,J)=" "
        F D=H:1:255 Q:$E(L,D)=" "
        S Z=M S:Z="Z" Z=$E(R,1,2) S IND=Z_" "_$E(L,J+1,D-1),^UTILITY("SUM1",%JB,%I,1,$E(IND,1,63))=IND
T2      S H=H+1
...
F1      S T=H,P=$E(L,H+1) F H=H+1:1 Q:$E(L,H)'?1A
        ...
        I P="V" S ^UTILITY("SUM1",%JB,%I,0,"$VIEW")="",^UTILITY("SUM1",%JB,0,0,"$VIEW")=""
...
G1      S F=1,C=H S:$E(L,C+1)="%" C=C+1 D C1 S:C["^" C=$P(C,"^",2)
        ...
        D P1:C'="" S S=0 G Q3:$E(L,H)'="(",F2:C'="" S ^UTILITY("SUM1",%JB,%I,0,"NAKED")="",^UTILITY("SUM1",%JB,0,0,"NAKED")="" G F2
C1      F H=C+1:1 Q:$E(L,H)'?1AN
        S C=$E(L,C,H-1) Q
V1      F H=H+1:1:255 D Q4:$E(L,H)=Q G Q3:$E(L,H)'?1AN,Q3:$E(L,H)="."
P1      D P2 S J=%I,%I=0 D P2 S %I=J Q
P2      Q:C'?1AP.E  S V="",%T=$E(T,1) Q:%T=""!'$F("VAGP",%T)  S %S=$E(T,2) I '$D(^UTILITY("SUM1",%JB,%I,%T,C)) S ^UTILITY("SUM1",%JB,%I,%T,C)=%S Q
        S V=^UTILITY("SUM1",%JB,%I,%T,C) I V="",%S'="" S ^UTILITY("SUM1",%JB,%I,%T,C)=%S Q
        I V'[%S S ^UTILITY("SUM1",%JB,%I,%T,C)=V_%S
        Q
Z       P %SUM1 ZS %SUM1
