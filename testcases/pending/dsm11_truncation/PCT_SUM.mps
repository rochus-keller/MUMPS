...
A2      I $D(^UTILITY("SUM1",%JB,0,0)) S %SUM=^(0) W *7,"  Recreate existing summary (",%SUM,")" R "  Ok? ",X G A6:X?1"Y".E,A1:X'?1"
?".E W !!,"  Enter Yes to recreate this summary or No to try another name",!! G A2
...
A5      S I=$N(^UTILITY("SUM1",I)) I I<0 G A1:J W *7,!,"   No documentation on file" G A1
        ...
        G A5
A6      R !!,"Do you want to print the summary? <Y> ",PRS G A1:PRS?1"^".E S:PRS="" PRS="Y" I PRS?1"?".E W !,"  Enter either Yes or N
o" G A6
A7      R !,"Do you want to print the routines? <Y> ",PRR G A6:PRR?1"^".E S:PRR="" PRR="Y" S %SYM=1 I PRR?1"?".E W !,"  Enter either
 Yes or No" G A7
...
INIT    K ^UTILITY("SUM",%JB),^UTILITY("SUM1",%JB),(%JB,%SUM,IOO,%IO,%DTY,STR,ARF,%OP,%DV,%BLK,%HEADER,PRR,PRS,%SYM) S A=0,^UTILITY(
"SUM",%JB)=$H,^UTILITY("SUM1",%JB,0,0)=%SUM
        X "F I=1:1 S A=$N(^UTILITY($J,A)),T=0 Q:A<0  ZL @A F J=1:1 S ^UTILITY(""SUM"",%JB,A,J)=$T(+J),T=T+$L($T(+J)) I $T(+J+1)=""""
 S ^UTILITY(""SUM1"",%JB,A,0)=T ZL %SUM Q"
...
KIL     K ^UTILITY($J),%A,%DT,%DAT,%DAT1,%DAT2,%SUM,%TIM,%TIM1,A,H,I,X,ARF,CRE,E,ERA,FIG,IN,INC,IND,IOM,K,L1,L2,LC,M,P,PGM,PRR,PRS,P
RT,R,STR,SW,SWT,TIM,TN,TYP,%BLK,%CH,%CP,%DTY,%DV,%FS,%HED,%II,%I9,%L9,%MS,%N,%NX,%OP,%PF,%ST,%SYM,%T,%T8,%TM,%TR
        K %UCN,%UCI
