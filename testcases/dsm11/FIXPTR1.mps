FIXPTR1 ;ASSEMBLE, FIND GLOBAL REFERENCE IN BLOCK @SMB@
        -
G       G R:GL="" K G W !,W R " Global ^",R Q:"^"[R  I R="?" D HELP G G
...
P       S C=$E(R,1),R=$E(R,2,999) I ","[C,'P G S
        ...
        -
R       K G W !,W R " Routine ",R Q:"^"[R
        I R'?1A.NA,R'?1"%".NA D HELP1 G R
        I $L(R)>8 D HELP1 G R
        F I=1:1:$L(R) S G(I+2)=I<$L(R)+($A(R,I)*2)
        S G(2)=64,G=I+2 G FIND
        -
...
NT      S L="" G P:C]""
FIND    S (P,R)=0,O=$V(1022,0),R1=2,U=-5
F1      S P=P+U+5,BYT=P,L=R1 G NF:P'<O S R=$V(P,0)#256,U=$V(P+1,0)#256
        G F1:R+2>R1 I R+2<R1 S R1=R+2 G NF
        S X=P-R,A=R+1+U
F2      S C=$V(X+R1,0)#256 G NF:C>G(R1),F1:C<G(R1) S R1=R1+1
...
DIS1    W !,$J(P,4),": " G Q:P=O I P>O W !,"Illegal offset!! Should be ",P,"." G Q
        ...
        E  F I=2:1:A D W
        W " = " S BYT=P D GETPTR W PTR S P=P+3 G DIS1
        -
...
O1      G NF:P'<O S BYT=P,R=$V(P,0)#256,U=$V(P+1,0)#256
        S A=R+U+1,X=P-R,P=P+U+5 F I=R+2:1:A S A(I)=$V(X+I,0)#256
        G O1:BYT<G,NF:BYT>G S (L,F)=0 W " " F I=2:1:A D W S G(I)=A(I)
