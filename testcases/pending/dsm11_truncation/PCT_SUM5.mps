...
A2      I $D(^UTILITY("SUM1",%JB,0,0)) S %SUM=^(0) W *7,"  Use existing summary (",%SUM,")" R "  Ok? ",X G ROU:X?1"Y".E,A1:X'?1"?".E
 W !!,"  Enter Yes to use this summary to reprint routines or No to try another name",!! G A2
ROU     D ^%SUMSEL Q:$N(^UTILITY($J,0))<0
SYM     R !,"Symbol table? <Yes> ",%SYM S:%SYM="" %SYM="Y" G %SUM5:%SYM?1"^".E I %SYM?1"?".E W "  Enter either Yes, No or <CR> for d
efault" G SYM
...
START   ;
        B 1 O %IO U %IO D:IOO'=%IO ^%HDR S CRE=^UTILITY("SUM",%JB),%DT=+CRE D %CDS^%H S %T=$P(CRE,",",2),CRE=%DAT1 D TIM D INT^%D,IN
T^%T S HEAD=" created on "_CRE_" at "_TIM_"     printed on "_%DAT1_" at "_%TIM1,TRM=%DTY="TRM",IOM='TRM*52+71
...
SB1     Q:'$D(^UTILITY("SUM1",%JB,PGM,0))  S NAM=PGM,SIZ=^(0) W !,NAM,?9,SIZ,?18,"   ",HEAD,!!
        F I=1:1 Q:'$D(^UTILITY("SUM",%JB,PGM,I))  S X=^(I),T=$P(X," ",1),X=$P(X," ",2,99) W !,T,?9,$E(X,1,IOM) F J=IOM+1:IOM-3:255 I
 $E(X,J,J+IOM-4)'="" W !,?7,".",?12,$E(X,J,J+IOM-4)
        G S1:'%SYM H:TRM 1 W #,!,NAM,?9,SIZ,?18,"   ",HEAD,!!
        F TYP="V","A","G","P" S V=-1 D SB2 W:TYP="G"&$D(^UTILITY("SUM1",%JB,PGM,0,"NAKED")) !!?2,"NAKED REFERENCES" W:TYP="V"&$D(^("
$VIEW")) !!?2,"$VIEW FUNCTION USED" W:TYP="V"&$D(^("VIEW")) !!?2,"VIEW COMMAND USED" W !!
S1      H:TRM 1 W # Q
