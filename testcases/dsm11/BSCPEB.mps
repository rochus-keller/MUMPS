...
STRT    ZA ^BSCDAT:1 I '$T S %MSN=1 D SNDMES D CHKHLT G SHTDWN:'$T,STRT
        ...
        S %CSET=^BSCDAT(%DEV,"STARTUP","CSET"),%LMOD=^("LMOD"),%NMOD=^("NMOD"),%EMUL=^("EMUL"),%GIN=^("GIN"),%LOG=^("LOG"),%TRN=^("TRN"),%RECSIZ=^("REC"),%SIGN=^("SIGNON")
...
INIDAT  F %I="SEND","RCVD","ERROR","SENT" I '$D(^BSCDAT(%DEV,%I)) S ^(%I)=0
        ...
        Q
LOGERR  F %I=1:1:$P($T(LOGTAB)," ",2) I @$P($T(LOGTAB+%I)," ",2) S %E=$E($T(LOGTAB+%I),4,8),^BSCDAT(%DEV,"STATUS",%E)=^BSCDAT(%DEV,"STATUS",%E)+1
        Q
