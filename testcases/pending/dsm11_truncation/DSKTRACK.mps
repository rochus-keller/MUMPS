...
COMP    ;
        ...
        F I=2:1 S TY=$P(TYS,";;",I) Q:TY=""  S P=$T(@TY),C=$P(P,";;",2),T=$P(P,";;",3),S=$P(P,";;",4),CR(TY)=+C,CH(TY)=$P(C,",",2),C
L(TY)=$P(C,",",3),TR(TY)=+T,TH(TY)=$P(T,",",2),TL(TY)=$P(T,",",3),SR(TY)=+S,SH(TY)=$P(S,",",2),SL(TY)=$P(S,",",3)
...
TM      S TM=$O(^SYS(0,"ERROR",TY,UNT,DT,TM)) G DT:TM=""
        S X=^(TM),BLK=$P(X,";",3),U=TY_UNT,C=$P(X,",",CR(TY))#PW(CH(TY)+1)\PW(CL(TY)),T=$P(X,",",TR(TY))#PW(TH(TY)+1)\PW(TL(TY)),S=$
P(X,",",SR(TY))#PW(SH(TY)+1)\PW(SL(TY))
...
B2      S B=$O(^SYS(0,"KTRBLK",U,B)) G B1:B="" S D=$O(^SYS(0,"KTRBLK",U,B,"")),DAY=$O(^(D)),TIM=$O(^(D,"")),T=$O(^(TIM)) G B2:DAY=""
&(T="")
        W !!,U,?8,B
        S (D,TM)=""
