RESTDO  Q  ;DSM11 UTILITIES; COPYRIGHT 1980 DEC
RE      ;
        S $ZT="ER" B 0 B 2
        ...
        W " LABEL ",M,! G S
BOTH    S MP=$V(812,0)
        S S=$V(DE)\256#64 I 'S S S=6*256 V DE::$V(DE)+S
        V DE+2::MP S MMS=$V(DE)\256#64+$V($V(44)+86)
        F I=0:2:190 V I:MMS:$V(512+I,0)
        I MP<MAPS W !,"Master disk is too small to recover the restored database",! G S
        I MBS U 63:(1:1:"C"),0 V 0:XD_XU V 914:0:$V(DE)#256+((DE-$V(ST+224)\32)*256)
        I MP'=MAPS W !,"WARNING: The target master disk is larger than the original master.",!
LABEL   S ME=MAPS*400+MB U 63:(1:2),0
        F I=496,498,704:2:811,904:2:983 V I+1024:0:$V(I,0)
        V 8+1024:0:$V(9,0)*256+($V(8+1024,0)#256)
        S V=$V(ST+310)\4*4+($V(ST+310)#2),WB=MB
        V ST+310::V+2 S $ZT="DEW" U 63:(2:1:"Z"),0 V -16777216:MD_MU U 63:(::"C"),0 S $ZT="ER" V ST+310::V
...
MOU     V DE::$V(DE)#16384+16384,DE+2::0
        U 63:(::"TZ"),0 V @B U 63 S A=$ZA\64#2 U 63:(::"C"),0 I A W !,"Disk not ready?" G MHA
        D DSM G:'M MHA
...
TERM    U 0 W !,"TERMINATED BY OPR.",! G REMNT
...
REM2    U 0 W !,"Please re-mount the original system disk:  ",MSY,!
        ...
        U 63:(1:1:"Z"),0 S RB=MB_":"_""""_MD_MU_"""" S:RESY<0 RB=XB_":"_""""_XD_XU_"""" V @RB U 63:(::"C"),0 S I1=816,SZ=22 D GTF GREM2:M'=MSY
RET     G FIN^RESTMNT
DEW     V ST+310::V
ER      U 0 G ER2:$ZE'?1"<DK".E
...
NX2     W !,"Please mount backup volume # ",VL," in drive ",XD,XU
        W "  *WRITE-PROTECTED*",!
NXR     R "then type  <CR>   ",A,! S %FAIL=0 I A="^" S %FAIL=1 Q
        I A'="" D MHLP G NX2
        U 63:(1:1:"Z"),0 S RB=XB V RB:XD_XU U 63:(::"C"),0 S RB=RB+'MBS
