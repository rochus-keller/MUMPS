...
V1      S SUBS=$N(^SYS(0,"PATCH",PATNO,SUBS))
        ...
        W !!,?10,"*** Both the old contents and new contents of patch #",PATNO,!,?14,"did not match current system image",! D HEAD W
 ?10,PATNO D START^AUPAT2 G NEXT
GET     S ERROR=0,PATDA=^SYS(0,"PATCH",PATNO,SUBS),MODUL=$P(PATDA,"/",1)
        I '$D(MEM(MODUL)) S ERROR=1 W ! W:OPT=4 !?10 W "*** Invalid module name: ",MODUL,", Patch #",PATNO," skipped" W:OPT'=4 " fro
m memory patches" W ! Q
...
G3      S MODUL=$P(%OD,"+",1),%OD=$P(%OD,"+",2) D ^%OD G:%OD="B" E2
        ...
        Q
E1      S ERROR=1 W ! W:OPT=4 ?10 W "*** Module ",MODUL," not in system, patch #",PATNO," skipped" W:OPT'=4 " from memory patches" W
 ! Q
...
E4      I OPT=4 W !,?10,PATNO,?26,"--" D START^AUPAT2 W !!,?10,"*** Note:  Patch #",PATNO," can only be applied to disk.",! S ERROR=
1 Q
