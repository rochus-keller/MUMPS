...
A1      F K=1:1:4 S Y=$P("G,P,A,V",",",K),N=999,L1=2,L2=8,SW=1 D:$Y>54 SB3:'SWT,SB4:SWT D SB1 S N=999,L1=1,L2=1 D SB1
        W !!,"VIEW USED" S J=0 F I=S+1:1:R S J=J+INC I $D(P(I)),$D(^UTILITY("SUM1",%JB,P(I),0,"VIEW")) W:INC=1 ?J-1*10+18 W:INC=2 ?J
+10 W "*"
...
S2      S T=T+1,TN=$N(^UTILITY("SUM1",%JB,TN)) I TN'=-1,$D(^UTILITY("SUM1",%JB,TN,0)) S P(T)=TN,E=E+1 G S2
        S T=T-1 Q:'E  S CRE=^UTILITY("SUM",%JB),%DT=+CRE D %CDS^%H S %T=$P(CRE,",",2),CRE=%DAT1 D TIM D INT^%D,INT^%T S HEAD=" creat
ed on "_CRE_" at "_TIM_"     printed on "_%DAT1_" at "_%TIM1
...
SB4     W # H 2 W NAM,?18,"   ",HEAD,! S (SW,%HED)=0
        ...
        W *13,$P("GLOBAL^ROUTINE^ARRAY^VARIABLE","^",K),!,$P("------^-------^-----^--------","^",K) S J=0 F I=S+1:1:R S J=J+INC W ?J
+10,"-"
        Q
