...
START   S CCEND=$V(ST+28)*64,INBLK=0,LINKTAB=ST+128
        ...
        K MOD,LINKTAB,SETDEF,OPT,INBLK,ADR,MODSTRT,CON,LIST,TAB,LOAD,USRADR,NAME,NEED,MODSUB,OPTSHO,DDBSIZ,DRVLN,SIZ,HLP,DDBSIZ,DDBM
IN
        D START^SGBUFF
RETURN  Q
TTYI    S DDBSIZ=2+^SYS(ID,"CONTROLLER","SINGLE")+(^("DH11")*16)+(^("DHU11")*16)+(^("DHV11")*8)+(^("DZ11")*8)+(^("DZV11")*4)*$V(ST+6
8)+DDBSIZ
