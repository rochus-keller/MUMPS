...
GC      I $V(ST+364)+$V(ST+380) H 1 G GC
        ...
        .I BBOF+3>BBSIZ W !,"Not enough bad block table space.  Re-boot and run SYSGEN",!,"to include ALL existing disks in your con
figuration.",! F I=1:1 H 0
        ...
        .S V3=^SYS(ID,"DDP","LINES",LINE,"V3"),NO=^("CONTROLLER NUMBER"),S=^("SERVICE"),CNT=^("CONTROLLER"),%OD=^SYS(ID,"CONTROLLER"
,CNT,NO,"CSR") D ^%OD S CSR=%OD
...
DEVS    ;
        ...
        F I=1:1:10 I $D(^SYS(ID,"TIED TERMINAL TABLE",I,"UCI")) S PG=I-1*4+ST+318 V PG::$E(^("VSET"),2)*32+^("UCI")*256+^("PARTITION
 SIZE"),PG+2::$A(^("ROUTINE NAME"),2)*256+$A(^("ROUTINE NAME"))
