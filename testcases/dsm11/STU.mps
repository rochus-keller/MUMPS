...
DEFQ    K ^SYS(0,"STARTUP") S ID=^("DEFAULT") I '$D(^SYS(ID,"STARTUP")) D ASKDEF Q:STBAS="Y"!(STBAS="^")  D ^STUBLD Q:ID="^"  D VERIFY G CMMND
        S QUES="DFLT" D ASKYN G:Y="^" START I Y D VERIFY G CMMND
BUILD   S QUES="BSLN" D ASKNY G DEFQ:Y="^",BASLIN:Y
CONF    R !,"Enter name of alternate configuration > ",CONF,! G BUILD:CONF=""!(CONF="^"),IDHLP:CONF="?" I '$D(^SYS(CONF)) W "Configuration ",CONF," does not exist.  Type '?' for help.",! G CONF
...
CMMND   D CHKVER^SYSROU G DEFQ:%A=0,SYSGEN:%A=1 D ^STUCSR G:$D(NOCSR) DEFQ D DATIM S ^SYS(0,"STARTUP","DATE-TIME")=DATIM S ZID=ID,ID=0 D WRTCMD^STUBLD2 S ID=ZID
...
VERIFY  K MAP,DRV,DSK
        ...
        S DSPL=^("DESPOOLER"),DSD=^("DEFAULT SPOOL DEVICE"),DDP=^("DDP"),LOADR=^("DRIVER"),MAP=^("ROUTINE MAP"),MNT=^("MOUNT"),%A=""        F I=0:0 S %A=$O(^SYS(ID,"STARTUP","DRIVER",%A)) Q:%A=""  S DRV(%A)=^(%A)
