...
DELIT   S %U="" I %A<0 S %X=$P(%L,"^",2) F %A=1:1:-%A D TOF:'%X Q:'%X  S:$L(%U)<100 %U=%X_","_%U S %Y=%X,%X=$P(^(%Y),"^",2) K ^(%Y)
        E  S:'%P %A=%A-1,%P=+%L,%L=^(%P) S %X=$P(%L,"^",2) F %A=1:1:%A D EOF:'%P Q:'%P  S:$L(%U)<100 %U=%P_","_%U S %Y=%P,%P=+%L,%L=
^(%P) K ^(%Y)
        I %X=%P D INIT
...
SAV     D STRIP S %I=$P(%A," ",1),%A=$P(%A," ",2,999)
        ...
        S %S=0,%U=@(%GL_"0)")
        I %P F %I=%I:-1:1 S %A=$P(%L,"^",3,999),%W=%P,%Z=%L,%P=%S,%L=%U D INS1:$D(@(%GL_"1)")) S %S=%P,%U=%L,%P=%W,%L=%Z Q:%I=1  D E
OF:'%L Q:'%L  S %P=+%L,%L=@(%G_"%P)")
        D RES Q
