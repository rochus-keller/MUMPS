...
MLOOP   .S DDU=$O(^SYS(0,"STARTUP","MOUNT",DDU))  Q:DDU=""
        ...
        S ^%Q("ER",35)=$P(^%ER,"\",2),(^SYS(0,"STARTUP","ERR JCDEV"),^%Q("ER",34))=$S($D(^SYS(ID,"STARTUP","ERR JCDEV")):^("ERR JCDE
V"),1:"N"),^%Q("ER",36)=$P(^%ER,"\",3),^%Q("ER",37)=$P(^%ER,"\",4)
...
DRIVER  I ^SYS(0,"STARTUP","DRIVER")="Y" D STU^LOADR
        ...
        ..I '$D(^SYS(0,"ROUTINE MAP",SETNAM)) K ^SYS(ID,"STARTUP","ROUTINE MAP",SETNAM) W !,"Routine map, ",SETNAM,", does not exist
 - deleted from Startup Command File",! Q
...
DSPLDO  S %FAIL=0,^SYS(0,"STARTUP","DESPOOLER")="N"
        ...
        S ^("DESPOOLER")="Y" K D Q
NSPLR   S ^SYS(0,"STARTUP","SPOOLING")="N" W !,"No SPOOL SPACE on disk, running with SPOOL device (#2) disabled",! I ^("DESPOOLER")=
"N" G NDFLT
