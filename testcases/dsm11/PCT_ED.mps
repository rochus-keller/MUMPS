...
%       X ^%(98) F %I=0:0 S %E=0 R !,"LINE> ",%T X:%T="." ^(99) Q:'$D(%T)  X:%T="?" ^(100),^(101),^(102) I %T'="?" X:%T=";" ^(6) I %T'=";" X:%T?1";".E ^(20) I %T'?1";".E X ^(0) S %E=%E!(%_%2+%O<0) X:%E ^(103) I '%E S:%1'="" %O=0 X ^(1),^(2) I '%E X ^(3)
0       S %=$S(%T["-":"-",%T["+":"+",1:""),%1=%T,%2="" S:%'="" %1=$P(%T,%,1),%2=$P(%T,%,2,255) S %E='(%1?1"%".AN!(%1?.AN)) Q:%E  S:%=""&(%2="") %="+",%2=0 S:%2=""&(%'="") %2=1 S %E=(%'=""&(%2'?1N.N))
1       S %O=(%_%2+%O) S:%1'="" %L=%1
2       S @("%0=$T("_%L_"+"_%O_")") S:%0'="" %X=%0 I %0="" S %E=1,%O=0 X ^(97)
3       W:%T="" %L_"+"_%O F %J=0:0 R "  r ",% W:%="" !,%X Q:%=""  X ^(4) W:%E "  Substring not found" I '%E R "  w ",% S %E=$L($E(%X,1,%1))+$L(%)+$L($E(%X,%2,255))>255 W:%E "  Too long" I '%E X ^(5) W:%E "   Invalid MUMPS line" I '%E
...
X ^(14) S %T=%TS,%X=%6
10      F %J=0:0 W !,$P("From\To","\",%S)," line> " R %T W:%T="" $P("First\Last","\",%S) S:%T="" %T=$S(%S=2:"+999",1:"+") X:%T="?" ^(104) I %T'="?" X ^(0) S:%="-" %E=1 X:%E ^(103) I '%E X:%S=1 ^(11) I '%E X:%S=2 ^(12) I '%E Q
...
16      S %S=0,%2=1 F %M=0:1 Q:'$F(%X,%,%2)  S:%TO'="""" %2=$F(%X,%,%2),%1=%2-$L(%)-1,%X=$E(%X,1,%1)_%TO_$E(%X,%2,255),%2=%1+$L(%TO)+1,%E='(%X?1"%".AN1" ".E!(%X?.AN1" ".E)),%S=1 I %TO="""" S %S=1 Q
20      W ! X $P(%T,";",2,$L(%T,";"))
97      W "   Line not found"
98      X ^(99) S (%L,%X)="",%O=0
99      K %,%1,%2,%E,%J,%I,%L,%O,%X,%0,%3,%4,%5,%6,%TS,%S,%TO,%EV,%K,%M,%T
100     W !!,"LINE entry:",!,"'.' to exit",!,"<CR> to edit the same line again",!,"';' to change every occurrence",!,"';' followed by a string of MUMPS code",!,"'TAG' OR 'TAG+/-Offset'",!,"'+/-Offset'",!
101     W !!,"REPLACE entry:",!,"Substring of line to be edited",!,"'END' to add onto end of line",!,"'substring1...substring2' to specify a long string",!,"<CR> to display the line",!
102     W !!,"WITH entry:",!,"Can be anything",!
103     W "  Incorrect response - Enter '?' for help"
104     W ?14,"Enter a line number, line number + offset,",!?14,"or <CR> to take the default of the ",$P("first\last","\",%S)," line of text"
END     Q
