...
A4      S X=$N(^UTILITY("SUM1",%JB,NAM,"P",X)) G A4:NAM=X I X<0 W:SWT ! G A6:'SW S SW=SW-1,TAB=TAB-18,X=Y(SW),NAM=KY(SW) S:TAB<18 TAB=18 G A4
...
A6      G B1:$D(G) S A=-1 F I=0:0:0 S A=$N(^UTILITY("SUM1",%JB,NAM,"G",A)) G B1:A<0 S B="" S:$D(G(A)) B=G(A) S G(A)=^(A)_B
        Q
B1      G B4:'$D(KO(O)),B4:'KO(O) S KO(O)=KO(O)-1,N=$N(KO(O,-1)) G B1:N<0 S Z=KO(O,N),O=O+1,NM=$P(Z,"[",1),C=$P(Z,"[",2),TAB=18,NAM=NM
...
B4      I O S O=O-1 G B1
        G:'$D(G) A2:ALL,ASK S TAB=10 W !!,"Globals:" S A=-1 F I=0:0 S A=$N(G(A)) G:A<0 A2:ALL,ASK S D=A W ?TAB,D W:G(A)["S" "*" S TAB=TAB+10 I TAB>70 S TAB=10 W !
        Q
HELP    S (I,J)=0 W !
