...
TST     D START^%STRTAB S STRNR=""
        F I=1:1 S STRNR=$O(STR(STRNR)) Q:STRNR=""  S NXVOL="" F J=1:1 S NXVOL=$O(STR(STRNR,NXVOL)) Q:NXVOL=""  G:$E(ARG2,2)=STRNR ST I $P(STR(STRNR,NXVOL),":",1)=ARG2 G ST
...
EXIT    C 63 S $ZT=ZTRAP U 0 I $D(%IOD) C:%IOD'=$I %IOD
        K %DTY,%DO,%IOD,A,ANS,AREA,BN,BTYP,BLOFF,BLN,C,CNT,COM,D,DATA,DEF,DL,HOLD,I,L,LSUB,MBLN,NAM,NXTBL,O63,P,PB,REF,SUB,UTLB,X,YI,YN Q
BYTE    S BYTES=1 G A
        -
