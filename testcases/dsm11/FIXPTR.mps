FIXPTR  ;REPAIR POINTER BLOCK @SMB@
        S W="" G GL:$V(0,0)#256 S R=$V(1,0) G GL:'R
        I $V(2,0)#256\2=32 S W="ROUTINES" G GL
        F I=1:1:R S W=W_$C($V(I+1,0)#256\2) Q:$V(I+1,0)#256#2=0
        I W'?1A.AN,W'?1"%".AN S W="" G GL
        I $L(W)>8 S W="" G GL
        S W="^"_W
GL      W !,"Pointer block for: " W:W]"" "<",W,"> " R " ",R S:R="" R=W
        I "^"[R W " (block not filed)" G B^FIX
        S GL="",NUMCOL=1 G R:$E("ROUTINES",1,$L(R))=R S GL=$E(R,2,999)
        I $L(R)<10,R?1"^"1A.AN!(R?1"^%".AN) G COL
        W " Enter R for routines, or ^NAME of some global" G GL
        -
...
O3      ;;LIST BLOCK
        S %QTY=2,%DEF=0 K %MTM D ^%IOS I $D(%IOD) U %IOD D DIS^FIXPTR1 U 0 C:%IOD'=$I %IOD
        G R
O4      V 1022:0:0 G DONE ;;CLEAR BLOCK;;1
O5      G INSERT ;;INSERT POINTER
O6      G DEL ;;ERASE POINTER
O7      ;;OFFSET
...
O9      ;;DOWN POINTER
        S W="FOR" D G^FIXPTR1 G R:'$D(G) S BYT=$V(BYT+1,0)#256+BYT+2 G PTR
        -
...
PUTBYT  I BYT#2 V BYT-1:0:A*256+($V(BYT-1,0)#256) Q
        ...
        -
INSERT  S M=1014-$V(1022,0)-6 I M<1 W !,"Block full" G R
        S W="Insert" D G^FIXPTR1 G R:'$D(G)
...
B       R !,"Pointer block # ",BT G INSERT:"^"[BT S B=$P(BT,":",1)
        ...
        S PTR=B,N=G-R1+4,O=$V(1022,0),R=9999
        ...
        S BYT=BYT+1 D SETPTR
        I R2>R S BYT=BYT+3,A=R2 D PUTBYT S BYT=BYT+1,A=U+R-R2 D PUTBYT
        W " Inserted" G INSERT
        -
DEL     S W="ERASE" D G^FIXPTR1 G R:'$D(G)
        S R1=$V(BYT,0)#256,N=$V(BYT+1,0)#256+5,O=$V(1022,0),BYT=BYT+N,R=0
