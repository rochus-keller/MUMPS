...
DEF     S LPR=0,CNTLER=^SYS(ID,"TTY",DEV,"CONTROLLER"),COM=^("COMMENT")
        ...
        S STALDF=^("STALL COUNT"),TABDF=^("TAB CONTROL"),MARDF=^("OUTPUT MARGIN"),LOWDF=^("LOWER CASE"),AUTDF=^("AUTOBAUD"),ZUSDF=^("ZUSE")
        ...
        I CNTLER="DH11" S CNTLER="DH"_(DH\16)_"-"_(DH#16),LPR=^("LPR"),C=$P($T(C),";;",2),RCVRDF=$P(C,",",LPR\64#16+1),XMITDF=$P(C,",",LPR\1024#16+1),DH=DH+1
        I CNTLER="DHU11" S CNTLER="DHU"_(DHU\16)_"-"_(DHU#16),LPR=^("LPR"),C=$P($T(D),";;",2),RCVRDF=$P(C,",",LPR\256#16),XMITDF=$P(C,",",LPR\4096),DHU=DHU+1
        I CNTLER="DHV11" S CNTLER="DHV"_(DHV\8)_"-"_(DHV#8),LPR=^("LPR"),C=$P($T(D),";;",2),RCVRDF=$P(C,",",LPR\256#16),XMITDF=$P(C,",",LPR\4096),DHV=DHV+1
        I CNTLER="DZ11"!(CNTLER="DZV11") S CNTLER="DZ"_(DZ\8)_"-"_(DZ#8),LPR=^("LPR"),B=$P($T(B),";;",2),RCVRDF=$P(B,",",LPR\256#16+1),XMITDF=RCVRDF,DZ=DZ+1
        S CONT="MUX" Q
PRCHAR  ;;
        I $Y>57 D LIN W #,!! D HDR2
        W ?1,DEV,?10,CNTLER W:CONT="MUX" ?20,RCVRDF,?26,XMITDF,?32,PARDF
        I CONT="MUX"!(CONT="DL") W ?40,DATADF,?46,STALDF,?52,OPDF,?59,TABDF,?66,LOWDF,?72,CRTDF,?78,LOGDF,?85,MARDF,?91,RTNDF,?96,AUTDF,?101,ZUSDF
        W ?104,COM,! D LIN Q
