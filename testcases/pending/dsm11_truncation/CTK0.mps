...
LOOP    I $V(ST+360)\2#2 ZU $V(ST+346)#256:(:::::32) W !!,*7,"*** System disk is off line  ***",! D CLR H TIM G LOOP
        ...
        I $V(ST+360) S ERR=$V(ST+360) D ZUSE^CTKUTL,JRN^CTK2:ERR#2,RDK^CTK2:ERR\16#2,HDK^CTK2:ERR\32#2,DKRES^CTK2:ERR\128#2,DBOVF^CT
K2:ERR\64#2,CLR^CTKUTL
...
NX      I JCDEV U JCDEV R P:0 I  G GER:P=$C(2),NX
        I $D(^SYS(0,"UNATTENDED BACKUP TIME")) S BTIM=^("UNATTENDED BACKUP TIME") I BTIM'="",+BTIM'>+$H,$P($P(BTIM,";",1),",",2)'>$P
($H,",",2) ZJ STR^BACKUP
        S ^SYS(0,"$HLAST")=$H
...
GER     R PN S H=+PN I '$D(^%ER(H)) S ^%ER(H)=1
        ...
        I ERR=MAXERR ZU $V(ST+346)#256:(:::::32) W !!,*7,"***  DSM error limit of ",ERR," reached !  D ^%ER to investigate." U JCDEV
G1      R Q
