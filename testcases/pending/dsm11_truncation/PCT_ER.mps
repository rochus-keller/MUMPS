...
T1      S X=$N(^%ER(DAT,X)) R Q:0 Q:Q'=""  G T2:X<0,T1:'($D(^(X))#2) S P=^(X) I P["<DSCON>",D S D=D+1 G T1
        S TM=$P($P(P,"\",1),",",2),W="AM",HH=TM\3600,R=TM-(3600*HH),MM=R\60,SS=R-(60*MM) S:HH>11 W="PM" S:HH>12 HH=HH-12 D:'(X-1#22)
 T3
        W !,$J(X,3),")  ",$P(P,"\",2),?39,$S(HH<10:0,1:""),HH,":",$S(MM<10:0,1:""),MM,":",$S(SS<10:0,1:""),SS," ",W,?52,$P(P,"\",7),
?61,$P(P,"\",3),?66,$P(P,"\",4) G T1
T2      I D W !!,$S(D-1:D-1,1:"No")," disconnect error",$S(D-1>1:"s",1:""),!
        Q
T3      W !!,?11,"$ZE",?41,"Time",?52,"UCI,VOL",?61,"$J",?66,"$I",! Q
DV      S P=^%ER(DAT,NUM) W ! F I=1:1:13 W:I#2 ! W ?I-1#2*40,$P("$H\$ZE\$J\$I\$ZA\$ZB\UCI,VOL\$X\$Y\$S of Symbols\$T\$ZV\Job Status"
,"\",I),?I-1#2*40+3," = ",$P(P,"\",I)
        Q
UDD     ;
        K DTE,DTH,ERR
...
RERR    U 0 W:$E($ZE,2)="Z" !,"Symbol table loaded"
        ...
        U 0 W !!,"Unable to restore environment.   $ZE = ",$ZE,!,"Symbol table cannot fit into available space or",!,"the routine th
at received the error no longer exists."
