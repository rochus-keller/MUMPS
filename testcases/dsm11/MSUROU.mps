...
D       ;
        ...
        S NXSTR=""
        F I=1:1 S NXSTR=$O(STR(NXSTR)) Q:NXSTR=""  S NXVOL="" F K=1:1 S NXVOL=$O(STR(NXSTR,NXVOL)) Q:NXVOL=""  I $P(STR(NXSTR,NXVOL),":",1)=(TYPE_U) G E
        G D1A
...
CONT    I '$D(STR(STRNR,K)) S ERROR=1 Q
        ...
        Q
HEADER  W !!,"Enter disk data as   DDU:M:BL",!,?10,"Where  DD = disk type",!,?17,"U  = unit no.",!,?17,"M  = map no.",!,?17,"BL = block no. within map",!
