...
CRT     W !,"Create a menu Entry",!
        ...
        S (ROU,HLP)=""
YN      I $D(@SB)#2 S ROU=$P(@SB,"]"),HLP=$P(@SB,"]",2) W !!,@SB,! R !,"Delete this entry ? [Y/N] ",ANS G:ANS="^" CRT G:(ANS=""!("YN"'[ANS)) YN I ANS="Y" K @SB W "  -  deleted"  G CRT
...
CRTDN   Q
%STT    I '$D(^UTILITY("MENU",$J,"CLR")) D ^%CURSOR S ^UTILITY("MENU",$J,"CLR")=%CUR("HOME")_","_%CUR("EES"),^("TYPE")=%CUR("TYPE"),^("Y")=%CUR("Y") K %CUR
...
LISTOPT W:^UTILITY("MENU",$J,"TYPE")["VT" # W @^UTILITY("MENU",$J,"CLR"),!,%T,!
        ...
        Q
LISTONE I ^UTILITY("MENU",$J,"TYPE")["VT" I $Y>(^UTILITY("MENU",$J,"Y")-4)&%H R !,?55,"Type <CR> to continue ",%R W #,@^UTILITY("MENU",$J,"CLR")
        W !,?5-$L(%I),%I,".",?8,%X S %Z=$P(@($E(^UTILITY("MENU",$J,"MENU"),1,$L(^("MENU"))-1)_","""_%X_""")"),"]",1) W:'(%Z="") "  ",?40,"(",%Z,")" S %Z=$P(^(%X),"]",2) D:%H HELPT
        Q
HELPT   Q:%Z=""  F %K=1:1 S %L=$P(%Z,"\",%K) Q:%L=""  W !,?8,%L
        W ! Q
HELPONE D ERROR Q
DOOPT   S %E=^(%X) I $E(%E,1)="]" D PUSH S ^UTILITY("MENU",$J,"MENU")=$E(^UTILITY("MENU",$J,"MENU"),1,$L(^("MENU"))-1)_","""_%X_""")" D %STT S %NOPAUSE=1 Q
...
END     I '(%POSTACT="") X %POSTACT
        ...
        D POP Q
PUSH    S:'$D(^UTILITY("MENU",$J,"MLEVEL")) ^("MLEVEL")=0 S ^("MLEVEL")=^("MLEVEL")+1,MLEVEL=^("MLEVEL"),^("MSTACK",MLEVEL)=^UTILITY("MENU",$J,"MENU") K MLEVEL Q
POP     S MLEVEL=^UTILITY("MENU",$J,"MLEVEL"),^("MLEVEL")=^("MLEVEL")-1,^UTILITY("MENU",$J,"MENU")=^("MSTACK",MLEVEL) K MLEVEL I '^("MLEVEL") K ^("MLEVEL"),^("MSTACK")
        Q
