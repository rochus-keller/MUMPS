IC1     ;CHECK ONE GLOBAL @SMB@
        D INIT Q:ER]""
LEV     S B=L(LEV),PE=L(LEV-1),T=2 S:LEV=1 T=8 S:LEV=2 T=6
...
P       I C#2 S L=$V(C+1,0)*256+$V(C,0)
        ...
        I L'=PE D PER
P1      S P=L U 63:(2:1) V P:STRNO I $V(1021,0)#128'=T D TYER
        S PE=$V(1020,0)#256*65536+$V(1018,0)
        I $V(0,0)#256 D NER
        I $V(1,0)+1'=A D NER
...
NX      I C'=O D OER
        ...
        -
INIT    C 63 O 63:(3) S A(0)=0,A(1)=$L(G) F I=1:1:$L(G)-1 S A(I+1)=$A(G,I)*2+1
        K T S T=0,$ZE="TRAP^IC2",B=0,ER="",A($L(G)+1)=$A(G,$L(G))*2
INIT1   V P:STRNO S T(T)=P F I=0:1:$L(G)+1 I $V(I,0)#256'=A(I) G INITER1
        S X=$V(1021,0)#128 I B G INIT2:X=8,INITER
...
TYER    S ER="Block type: expected "_T_" got "_($V(1021,0)#128) G ERP
PER     S ER="Right pointer: expected "_L_" got "_PE G ERP
MER     I $V(P-S*2,0) S ER="Illegal map block entry "_$V(P-S*2,0) G ERP
        S ER="In structure and free" G ERP
RER     S ER="Bad repeat count" G ERP
UER     S ER="Uncommon char. count=0" G ERB
