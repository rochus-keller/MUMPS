...
SYM     R !,"Symbol table? <Yes> ",%SYM S:%SYM="" %SYM="Y" G %CRF:%SYM?1"^".E I %SYM?1"?".E W "  Enter either Yes, No or <CR> for de
fault" G SYM
...
A1      I $D(^UTILITY("CRF",%JB))!$D(^UTILITY("CRF1",%JB)) S %JB=%JB+100 G A1
        ...
        X "F I=1:1 S A=$N(^UTILITY($J,A)),T=0 Q:A<0  ZL @A S %R=%R+1 F J=1:1 S ^UTILITY(""CRF"",%JB,I,J)=$T(+J),T=T+$L($T(+J)) I $T(
+J+1)="""" S ^UTILITY(""CRF1"",%JB,I,0)=A_""^""_T ZL %CRF Q"
        I %SYM F %I=1:1:%R D ^%CRF1
        S:'$D(%DTY) %DTY="TRM" W:IOO=%IO # O:%IO<59!(%IO>63) %IO O:%IO>58&(%IO<63) %IO:(0:%BLK) D PRT
A2      S $ZT="" K ^UTILITY($J),%DT,%DAT,%DAT1,%DAT2,%R,%SYM,%TIM,%TIM1,A,I,IOM,X,%I9,%L9,%MS,%NX,%PF,%ST,%T8,%TM,%TR,%UCI,%UCN,ARF,
ERA,FIG,LC,STR,%BLK,%CH,%CP,%DTY,%DV,%FS
...
$E(X,J,J+IOM-4)'="" W !,?7,".",?12,$E(X,J,J+IOM-4)
        ...
        F X=1:1:4 S V=999,Y=$P("V,A,G,P",",",X),W=Y_"S" D SB2 W:X=3&$D(^UTILITY("CRF1",%JB,%I,0,"NAKED")) !!?2,"NAKED REFERENCES" W:
X=1&$D(^("$VIEW")) !!?2,"$VIEW FUNCTION USED" W:X=1&$D(^("VIEW")) !!?2,"VIEW COMMAND USED" W !!
S1      H:TRM 1 W # Q
