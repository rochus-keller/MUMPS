...
%SCR    S %LN=132 S:%DTY="TRM" %LN=80 I %DTY'="TRM" S %SC=0 G %ASKC
        R !,"Scroll ? <N> ",%SC:%TMO G:%SC="?" %Q1 G:%SC="^"!('$T) %STL G:%SC="^Q" %END S:%SC="" %SC="N" I "Y^N"'[$E(%SC,1) W "   'Y' or 'N'" G %SCR
        S %SC=($E(%SC,1)="Y"),%PAG=20
%PAG    G:'%SC %ASKC W !,"Lines/Page <",%PAG R "> ",%X:%TMO G:%X="^"!('$T) %SCR G:%X="^Q" %END S:%X="" %X=%PAG I %X'?1N.N!(%X<1) G %Q2
        S %PAG=%X
%ASKC   R !,"Do you want to display control characters ? <NO> ",%X:%TMO G:%X="?" %Q3 G:%X="^Q" %END S:%X="" %X="NO" I %X="^"!('$T) G:%SC %PAG G:%DTY'="TRM" %STL G %SCR
...
%END    I $D(%IOD) C:%IOD'=%TRM %IOD
        K %ASCII,%BAR,%CHR,%CK,%DCC,%DCF,%DEF,%DTY,%FCR,%GL,%GN,%GO,%IOD,%K,%LC,%LIN,%LN,%NCR,%NLN,%PAG,%SC,%ST,%TMO,%TRM,%UCIN,%X,A,GLREF,I,IN,K,M Q
