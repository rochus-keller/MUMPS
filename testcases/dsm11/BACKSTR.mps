...
ASK     S %TM=%T W !,"Is this " D CVT^%T W " in the ",$S(%T<43200:"Morning",%T>43199&(%T<64800):"Afternoon",1:"Evening") R " ? <Y> ",A S:A="" A="Y" G TIM:A'?1"Y".E
        S TIMFL=^SYS(0,"UNATTENDED BACKUP TIME")
        F I=1:1 S DATIM=$P(TIMFL,";",I) Q:DATIM=""!($P(DATIM,",",1)>H)  I $P(DATIM,",",1)=H Q:$P(DATIM,",",2)'<%T
        S ^("UNATTENDED BACKUP TIME")=$P(^("UNATTENDED BACKUP TIME"),";",1,I-1)_$S(I-1:";",1:"")_H_","_%T_";"_$P(^("UNATTENDED BACKUP TIME"),";",I,999)
        S ^SYS(0,"UNATTENDED BACKUP FILE")=$P(^SYS(0,"UNATTENDED BACKUP FILE"),";",1,I-1)_$S(I-1:";",1:"")_NM_";"_$P(^("UNATTENDED BACKUP FILE"),";",I,999)
